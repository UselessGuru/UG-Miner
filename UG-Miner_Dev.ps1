...
# Added temporary, set a trace point6
While ($Variables.NewMiningStatus -eq "Running") { 

    If (Get-Variable ButtonPause -ErrorAction Ignore) { $ButtonPause.Enabled = $false }
    If (Get-Variable ButtonStart -ErrorAction Ignore) { $ButtonStart.Enabled = $false }
    ...
    Try { 
        If ($BrainNameToDebug = "") { 
            $Variables.Brains.$BrainNameToDebug = @{ }
        ...
        $Variables.MiningStatus = "Running"
        . .\Includes\Core_Dev.ps1
        # . .\Includes\Core_Dev_v4.ps1
        # . .\Includes\Core.ps1
        ForEach ($Miner in $Variables.Miners.Where({ $_.ProcessId })) { 
        ...
        If ($Config.ShowConsole) { 
            ...
            If ($Variables.MinersBestPerDevice_Combo) { 
                ...
                [System.Collections.ArrayList]$Miner_Table = @(
                    ...
                    @{ Label = "Cnt"; Expr6ession = { Switch ($_.Activated) { 0 { "Never" } 1 { "Once" } Default { "$_" } } } }

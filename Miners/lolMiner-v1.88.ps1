...
#   @{ Algorithms = @("Cuckatoo32");                   Type = "NVIDIA"; Fee = @(0.02);       MinMemGiB = 4.0;  MinerSet = 3; WarmupTimes = @(60, 60);  ExcludeGPUarchitectures = " ";                ExcludePools = @(@(), @());                         Arguments = " --algo C32" } # ASIC
    ...
    @{ Algorithms = @("EtcHash", "Blake3");            Type = "NVIDIA"; Fee = @(0.01, 0);    MinMemGiB = 1.24; MinerSet = 2; WarmupTimes = @(60, 100); ExcludeGPUarchitectures = " ";                ExcludePools = @(@(), @());                         Arguments = " --algo ETCHASH --dualmode ALEPHDUAL --maxdualimpact *" } # No hashrate for second algorithm
#   @{ Algorithms = @("EtcHash", "HeavyHashKarlsen");  Type = "NVIDIA"; Fee = @(0.01, 0.01); MinMemGiB = 1.24; MinerSet = 1; WarmupTimes = @(60, 100);  ExcludeGPUarchitectures = " ";                ExcludePools = @(@(), @());                         Arguments = " --algo ETCHASH --dualmode KARLSENDUAL --maxdualimpact *" } # No hashrate for second algorithm
#   @{ Algorithms = @("EtcHash", "SHA512256d");        Type = "NVIDIA"; Fee = @(0.01, 0.01); MinMemGiB = 1.24; MinerSet = 1; WarmupTimes = @(45, 100);  ExcludeGPUarchitectures = " ";                ExcludePools = @(@(), @());                         Arguments = " --algo ETCHASH --dualmode RXDDUAL --maxdualimpact *" } # https://github.com/Lolliedieb/lolMiner-releases/issues/1986
...
#   @{ Algorithms = @("Ethash", "SHA512256d");         Type = "NVIDIA"; Fee = @(0.01, 0.01); MinMemGiB = 1.24; MinerSet = 2; WarmupTimes = @(60, 100);  ExcludeGPUarchitectures = " ";                ExcludePools = @(@(), @());                         Arguments = " --algo ETHASH --dualmode RXDDUAL --maxdualimpact *" } # https://github.com/Lolliedieb/lolMiner-releases/issues/1986

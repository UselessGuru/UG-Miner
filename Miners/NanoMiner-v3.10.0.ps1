...
$Algorithms = @(
    ...
    @{ Algorithms = @("KawPow", "");             Type = "AMD"; Fee = @(0.02);  MinMemGiB = 1.08; MinerSet = 2; Tuning = " -coreClocks +20 -memClocks +100 -memTweak 2"; WarmupTimes = @(45, 0);  ExcludeGPUarchitectures = " "; ExcludePools = @(@(), @());           Arguments = @(" -algo KawPow") } # TeamRedMiner-v0.10.21 is fastest
...
#   @{ Algorithms = @("Randomx", "");    Type = "CPU"; Fee = @(0.02); MinerSet = 3; WarmupTimes = @(45, 0); ExcludeGPUarchitectures = " "; ExcludePools = @(@(), @()); Arguments = @(" -algo Randomx") } # ASIC
    ...
    @{ Algorithms = @("KawPow", "");             Type = "INTEL"; Fee = @(0.02); MinMemGiB = 1.08; MinerSet = 2; WarmupTimes = @(45, 0);  ExcludeGPUarchitectures = " "; ExcludePools = @(@(), @()); Arguments = @(" -algo KawPow") } # Trex-v0.26.8 is fastest
    ...
    @{ Algorithms = @("KawPow", "");             Type = "NVIDIA"; Fee = @(0.02);  MinMemGiB = 1.08; MinerSet = 0; Tuning = " -coreClocks +20 -memClocks +100"; WarmupTimes = @(45, 0);  ExcludeGPUarchitectures = " ";       ExcludePools = @(@(), @());           Arguments = @(" -algo KawPow") } # Trex-v0.26.8 is fastest

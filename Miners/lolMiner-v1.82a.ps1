...
#   [PSCustomObject]@{ Algorithms = @("Cuckatoo32");                 Type = "AMD"; Fee = @(0.02);       MinMemGiB = 4.0;  Minerset = 3; WarmupTimes = @(60, 70); ExcludeGPUArchitecture = " ";         ExcludePools = @(@(), @()); Arguments = " --algo C32" } # ASIC
    ...
    [PSCustomObject]@{ Algorithms = @("EtcHash", "kHeavyHash");      Type = "AMD"; Fee = @(0.01, 0.01); MinMemGiB = 1.24; Minerset = 2; WarmupTimes = @(90, 90); ExcludeGPUArchitecture = "^RDNA1$";   ExcludePools = @(@(), @()); Arguments = " --algo ETCHASH --dualmode KASPADUAL" } # https://github.com/Lolliedieb/lolMiner-releases/issues/1987

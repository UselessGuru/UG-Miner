...
#   @{ Algorithms = @("Cuckatoo32", "");                 Type = "AMD"; Fee = @(0.02);        MinMemGiB = 4.0;  WarmupTimes = @(60, 70);  ExcludeGPUarchitectures = " ";        ExcludePools = @(@(), @());           Arguments = " --algo C32" } # ASIC
    ...
    @{ Algorithms = @("BeamV3", "");                             Type = "NVIDIA"; Fee = @(0.01);        MinMemGiB = 3.0;  WarmupTimes = @(45, 50);  ExcludeGPUarchitectures = "^Other$|^Pascal$|^Turing$"; ExcludePools = @(@(), @());           Arguments = " --algo BEAM-III" }

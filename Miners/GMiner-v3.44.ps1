...
#   @{ Algorithms = @("Cuckatoo32", "");   Type = "AMD"; Fee = @(0.05);  MinMemGiB = 8.0;  Tuning = ""; MinerSet = 3; WarmupTimes = @(30, 0);  ExcludePools = @(@(), @());             AutoCoinPers = "";             Arguments = " --algo cuckatoo32 --cuda 0 --opencl 1" } # ASIC
    @{ Algorithms = @("Equihash1254", ""); Type = "AMD"; Fee = @(0.02);  MinMemGiB = 2.1;  Tuning = ""; MinerSet = 0; WarmupTimes = @(45, 0);  ExcludePools = @(@(), @());             AutoCoinPers = "";             Arguments = " --algo equihash125_4 --cuda 0 --opencl 1" } # lolMiner-v1.97 is fastest

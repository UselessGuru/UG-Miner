...
$Algorithms = @(
    [PSCustomObject]@{ Algorithms = @("EtcHash");            Type = "AMD"; Fee = @(0.0065);   MinMemGiB = 0.77; MinerSet = 0; Tuning = " -mi 12 -straps 1"; WarmupTimes = @(45, 15); ExcludeGPUArchitecture = @();                 ExcludePools = @(@(), @()); Arguments = " -amd -eres 1 -coin ETC" } # GMiner-v3.42 is just as fast, PhoenixMiner-v6.2c is maybe faster, but I see lower speed at the pool
    ...
    [PSCustomObject]@{ Algorithms = @("Ethash");             Type = "AMD"; Fee = @(0.0065);   MinMemGiB = 0.77; MinerSet = 0; Tuning = " -mi 12 -straps 1"; WarmupTimes = @(45, 15); ExcludeGPUArchitecture = @();                 ExcludePools = @(@(), @()); Arguments = " -amd -eres 1" } # GMiner-v3.42 is just as fast, PhoenixMiner-v6.2c is maybe faster, but I see lower speed at the pool
    ...
    [PSCustomObject]@{ Algorithms = @("EtcHash");            Type = "NVIDIA"; Fee = @(0.0065);   MinMemGiB = 0.77; MinerSet = 1; Tuning = " -mi 12 -vmt1 15 -vmt2 12 -vmt3 0 -vmr 15 -mcdag 1"; WarmupTimes = @(45, 15); ExcludePools = @(@(), @()); Arguments = " -nvidia -eres 1 -coin ETC" } # GMiner-v3.42 is just as fast, PhoenixMiner-v6.2c is maybe faster, but I see lower speed at the pool
    ...
    [PSCustomObject]@{ Algorithms = @("Ethash");             Type = "NVIDIA"; Fee = @(0.0065);   MinMemGiB = 0.77; MinerSet = 1; Tuning = " -mi 12 -vmt1 15 -vmt2 12 -vmt3 0 -vmr 15 -mcdag 1"; WarmupTimes = @(45, 15); ExcludePools = @(@(), @()); Arguments = " -nvidia -eres 1" } # GMiner-v3.42 is just as fast, PhoenixMiner-v6.2c is maybe faster, but I see lower speed at the pool
            ...
            If ($Miner_Devices = $Devices | Where-Object Type -EQ $_.Type | Where-Object Model -EQ $_.Model) { 
                ...
                ($Algorithms | Where-Object Type -EQ $_.Type).ForEach(
                    { 
                        ...
                        ForEach ($Pool0 in ($MinerPools[0][$_.Algorithms[0]].Where({ $_.Name -notin $ExcludePools[0] }).Where({ $_.Epoch -lt 602 }).Where({ $_.Algorithm -ne "EtcHash" -or $_.Epoch -lt 302 }))) { 
                            ForEach ($Pool1 in ($MinerPools[1][$_.Algorithms[1]].Where({ $_.Name -notin $ExcludePools[1] }))) { 
                                ...
                                If ($AvailableMiner_Devices = $Miner_Devices.Where({ $_.MemoryGiB -ge $MinMemGiB }).Where({ $_.Architecture -notin $ExcludeGPUArchitecture })) { 
                                    ...
                                    If ($AvailableMiner_Devices) { 
                                        ...
                                        [PSCustomObject]@{ 
                                            API         = "EthMiner"
                                            Arguments   = "$Arguments -vmdag 0 -log 0 -wdog 0 -cdmport $MinerAPIPort -gpus $(($AvailableMiner_Devices.$DeviceEnumerator | Sort-Object -Unique).ForEach({ '{0:d}' -f ($_ + 1) }) -join ',')"

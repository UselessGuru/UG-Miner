...
$URI = "https://github.com/sp-hash/TeamBlackMiner/releases/download/v2.17/TeamBlackMiner_2_17_cuda_12_2.7z"
...
$Algorithms = @(
    ...
    [PSCustomObject]@{ Algorithms = @("EtcHash", "EthashB3");  Type = "AMD"; Fee = @(0.005, 0.005); MinMemGiB = 1.51; MinerSet = 2; Tuning = ""; WarmupTimes = @(120, 45); ExcludeGPUArchitecture = @(); ExcludePools = @(@(), @()); Arguments = " --algo etc+ethb3" } 
    ...
    [PSCustomObject]@{ Algorithms = @("Ethash", "EthashB3");   Type = "AMD"; Fee = @(0.005, 0.005); MinMemGiB = 1.51; MinerSet = 2; Tuning = ""; WarmupTimes = @(120, 15); ExcludeGPUArchitecture = @(); ExcludePools = @(@(), @()); Arguments = " --algo eth+ethb3" }
                        ...
                        ForEach ($Pool1 in ($MinerPools[1][$_.Algorithms[1]].Where({ $_.Name -notin $ExcludePools[1] }))) { 
                            ...
                            If (-not $_.Algorithms[1] -or ($Pool0.PoolPorts[0] -and $Pool1.PoolPorts[0]) -or ($Pool0.PoolPorts[1] -and $Pool1.PoolPorts[1])) { 
                                ...
                                If ($AvailableMiner_Devices = $Miner_Devices.Where({ $_.MemoryGiB -ge $MinMemGiB -and $_.Architecture -notin $ExcludeGPUArchitecture })) { 
                                    ...
                                    # $Miner_Name = "$Name-$($AvailableMiner_Devices.Count)x$($AvailableMiner_Devices.Model | Select-Object -Unique)$(If ($Pool1) { "-$($Pool0.AlgorithmVariant)&$($Pool1.AlgorithmVariant)" })$(If ($_.Intensity) { "-$($_.Intensity)" })"
                                    ...
                                    [PSCustomObject]@{ 
                                        API              = "TeamBlackMiner"
                                        Arguments        = "$Arguments --api --api-version 1.4 --api-port $MinerAPIPort $DeviceSelector [$(($AvailableMiner_Devices.$DeviceEnumerator | Sort-Object -Unique | ForEach-Object { '{0:x}' -f $_ }) -join ',')]"

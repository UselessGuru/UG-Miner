...
$Algorithms = @( 
    @{ Algorithm = "Ethash"; Type = "AMD"; Fee = @(0.006); MinMemGiB = 0.77; MinerSet = 2; Tuning = " -rxboost 1"; WarmupTimes = @(60, 0); ExcludePools = @(); Arguments = " -platform 1" } # PhoenixMiner-v6.2c may be faster, but I see lower speed at the pool

    @{ Algorithm = "Ethash"; Type = "NVIDIA"; Fee = @(0.006); MinMemGiB = 0.77; MinerSet = 2; Tuning = " -strap 1"; WarmupTimes = @(60, 0); ExcludePools = @(); Arguments = " -platform 2" } # PhoenixMiner-v6.2c may be faster, but I see lower speed at the pool
)
            ...
            If ($MinerDevices = $Devices | Where-Object Type -EQ $_.Type | Where-Object Model -EQ $_.Model) { 
                ...
                ($Algorithms | Where-Object Type -EQ $_.Type).ForEach(
                    { 
                        ...
                        # ForEach ($Pool in $MinerPools[0][$_.Algorithm].Where({ $_.Name -notin $ExcludePools -and $_.Epoch -le 600 })) { 
                        ForEach ($Pool in $MinerPools[0][$_.Algorithm].Where({ $_.Epoch -le 600 })) { 
                            ...
                            If ($AvailableMinerDevices = $MinerDevices.Where({ $_.MemoryGiB -ge $MinMemGiB })) { 
                                ...
                                [PSCustomObject]@{ 
                                    API         = "EthMiner"
                                    Arguments   = "$Arguments -dbg -1 -wd 0 -retrydelay 3 -allpools 1 -allcoins 1 -mport -$MinerAPIPort -di $(($AvailableMinerDevices.$DeviceEnumerator | Sort-Object -Unique).ForEach({ '{0:x}' -f $_ }) -join ',')"

...
$URI = "https://github.com/UselessGuru/UG-Miner-Binaries/releases/download/BzMiner/bzminer_v21.0.3_windows.zip"
...
$Algorithms = @(
    ...
    [PSCustomObject]@{ Algorithms = @("Ethash3B");         Type = "AMD"; Fee = @(0.01);       MinMemGiB = 1.08; MinerSet = 1; Tuning = " --oc_mem_tweak 2"; WarmupTimes = @(45, 20); ExcludeGPUArchitecture = @("GCN4"); ExcludeGPUModel = ""; ExcludePools = @(@(), @());             Arguments = @(" -a rethereum") } # AMD polaris cards (eg. rx470, rx580) may not work with rethereum right now. Looking into this soon
...
#   [PSCustomObject]@{ Algorithms = @("HeavyHashKaspa");               Type = "NVIDIA"; Fee = @(0.01);       MinMemGiB = 2;    MinerSet = 2; Tuning = " --oc_mem_tweak 2"; WarmupTimes = @(45, 15); ExcludeGPUArchitecture = @();        ExcludeGPUModel = "^MX[1|2]\d+"; ExcludePools = @(@("ProHashing"), @());           Arguments = @(" -a kaspa") } # ASIC

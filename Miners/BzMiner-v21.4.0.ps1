...
$URI = "https://github.com/UselessGuru/UG-Miner-Binaries/releases/download/BzMiner/bzminer_v21.4.0_windows.zip"
...
#   @{ Algorithms = @("HeavyHashKaspa");   Type = "AMD"; Fee = @(0.01);  MinMemGiB = 2;    MinerSet = 2; Tuning = " --oc_mem_tweak 2"; WarmupTimes = @(45, 15); ExcludeGPUarchitectures = @();        ExcludeGPUModel = ""; ExcludePools = @(@("ProHashing"), @());             Arguments = @(" -a kaspa") } # ASIC
    ...
    @{ Algorithms = @("FishHash");         Type = "INTEL"; Fee = @(0.01);  MinMemGiB = 1.24; MinerSet = 1; Tuning = " --oc_mem_tweak 2"; WarmupTimes = @(60, 60); ExcludeGPUarchitectures = @(); ExcludeGPUModel = ""; ExcludePools = @(@("NiceHash", "NiceHash"), @()); Arguments = @(" -a ironfish") } # https://github.com/bzminer/bzminer/issues/260
    @{ Algorithms = @("HeavyHashKarlsen"); Type = "INTEL"; Fee = @(0.01);  MinMemGiB = 1.08; MinerSet = 1; Tuning = " --oc_mem_tweak 2"; WarmupTimes = @(45, 0);  ExcludeGPUarchitectures = @(); ExcludeGPUModel = ""; ExcludePools = @(@("ZergPool"), @());             Arguments = @(" -a karlsen") }
...
#   @{ Algorithms = @("Autolykos2", "HeavyHashKaspa"); Type = "NVIDIA"; Fee = @(0.01, 0.01); MinMemGiB = 1.24; MinerSet = 1; Tuning = " --oc_mem_tweak 2"; WarmupTimes = @(45, 60); ExcludeGPUarchitectures = @();        ExcludeGPUModel = "^MX[1|2]\d+"; ExcludePools = @(@("NiceHash", "ProHashing"), @()); Arguments = @(" -a ergo", " --a2 kaspa") } # ASIC
    ...
    @{ Algorithms = @("Blake3");                       Type = "NVIDIA"; Fee = @(0.005);      MinMemGiB = 2;    MinerSet = 1; Tuning = " --oc_mem_tweak 2"; WarmupTimes = @(45, 15); ExcludeGPUarchitectures = @();        ExcludeGPUModel = "";            ExcludePools = @(@(), @());                         Arguments = @(" -a alph") }
...
#   @{ Algorithms = @("HeavyHashKaspa");               Type = "NVIDIA"; Fee = @(0.01);       MinMemGiB = 2;    MinerSet = 2; Tuning = " --oc_mem_tweak 2"; WarmupTimes = @(45, 15); ExcludeGPUarchitectures = @();        ExcludeGPUModel = "^MX[1|2]\d+"; ExcludePools = @(@("NiceHash"), @());               Arguments = @(" -a kaspa") } # ASIC
    ...
    @{ Algorithms = @("SHA512256d");                   Type = "NVIDIA"; Fee = @(0.01);       MinMemGiB = 1;    MinerSet = 1; Tuning = " --oc_mem_tweak 2"; WarmupTimes = @(45, 15); ExcludeGPUarchitectures = @();        ExcludeGPUModel = "^MX[1|2]\d+"; ExcludePools = @(@(), @());                         Arguments = @(" -a radiant") }
    @{ Algorithms = @("SHA256dt");                     Type = "NVIDIA"; Fee = @(0.01);       MinMemGiB = 1;    MinerSet = 1; Tuning = " --oc_mem_tweak 2"; WarmupTimes = @(45, 15); ExcludeGPUarchitectures = @();        ExcludeGPUModel = "^MX[1|2]\d+"; ExcludePools = @(@(), @());                         Arguments = @(" -a novo") }
    @{ Algorithms = @("SHA3d");                        Type = "NVIDIA"; Fee = @(0.01);       MinMemGiB = 1;    MinerSet = 1; Tuning = " --oc_mem_tweak 2"; WarmupTimes = @(45, 15); ExcludeGPUarchitectures = @();        ExcludeGPUModel = "^MX[1|2]\d+"; ExcludePools = @(@(), @());                         Arguments = @(" -a kylacoin") }
    @{ Algorithms = @("Skein2");                       Type = "NVIDIA"; Fee = @(0.01);       MinMemGiB = 2;    MinerSet = 1; Tuning = " --oc_mem_tweak 2"; WarmupTimes = @(45, 15); ExcludeGPUarchitectures = @();        ExcludeGPUModel = "";            ExcludePools = @(@(), @());                         Arguments = @(" -a woodcoin") }
)

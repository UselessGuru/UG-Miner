...
#   [PSCustomObject]@{ Algorithms = @("Cuckatoo32");                Type = "NVIDIA"; Fee = @(0.05);       MinMemGiB = 8.0;  Tuning = " --mt 2"; MinerSet = 3; WarmupTimes = @(30, 0);  ExcludeGPUArchitecture = @();        ExcludePools = @(@(), @());             AutoCoinPers = "";             Arguments = " --nvml 0 --algo cuckatoo32 --cuda 1 --opencl 0" } # ASIC
    ...
    [PSCustomObject]@{ Algorithms = @("Ethash", "HeavyHashKaspa");  Type = "NVIDIA"; Fee = @(0.01, 0.01); MinMemGiB = 1.24; Tuning = " --mt 2"; MinerSet = 2; WarmupTimes = @(45, 15); ExcludeGPUArchitecture = @("Other"); ExcludePools = @(@(), @());             AutoCoinPers = "";             Arguments = " --nvml 0 --algo ethash --dalgo kheavyhash --cuda 1 --opencl 0" }

...
# Warthog: Added community fund, 2% minimum (can increase with community_fund options). Dev Fund address: 257edaceb6cb5ded59afd2051b93c5244053da527fc28d6a
# Lots of backend updates around cpu thread and memory management
# Bug fix: Warthog reconnects causing crashes
# Bug fix: Dual cpu reporting same temperature for both cpus
# Bug fix: Linux power reporting sometimes drops to 0W
# Bug fix: bz should no longer scan for all storage devices
# Warthog: Added 2 decimal places for sha precision
# Warthog: Added worker name to rpc protocol
# Warthog: Fixed dual cpu with disabled cpu (or cpu on another algo) max memory option resulting in lower memory allocated than expected
# Options: Added cpu instruction set and huge pages options (disable_sse, disable_avx, disable_avx512, disable_huge_pages)
# Updated watchdog_no_new_work_seconds option to include work id (if same job is repeatedly sent, generally meaning bug in node or pool, currently warthog only, but can add others on request)
# Bug fix: --enable, --disable, config.txt start_mining option for dual cpu rigs now work for cpu algos (devices 255:0, 255:1)
# Bug semi-fix: Disabled windows key input when bz is not in focus (completely disabled key inputs until later release fixes root issue)
# Bug fix: Pure verus crashing on Linux
# Linux: Removed X11 warning
...
$URI = "https://github.com/UselessGuru/UG-Miner-Binaries/releases/download/BzMiner/bzminer_v23.0.4_windows.zip"

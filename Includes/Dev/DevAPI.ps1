...
Switch ($Path) { 
    ...
    "/functions/balancedata/remove" { 
        If ($Parameters.Data) { 
            $BalanceDataEntries = ($Parameters.Data | ConvertFrom-Json -ErrorAction Ignore)
            $Variables.BalancesData = @((Compare-Object $Variables.BalancesData $BalanceDataEntries -PassThru -Property DateTime, Pool, Currency, Wallet) | Select-Object -ExcludeProperty SideIndicator)
            $Variables.BalancesData | ConvertTo-Json | Out-File ".\Logs\BalancesTrackerData.json"
    ...
    "/functions/mining/pause" { 
        If ($Variables.MiningStatus -ne "Paused") { 
            ...
            $Data = "Mining is being paused...`n$(If ($Variables.BalancesTrackerPollInterval -gt 0) { If (-not $BalancesTrackerRunspace) { "Balances Tracker starting..." } Else { "Balances Tracker running." } })"
            $Variables.RestartCycle = $true
        }
        $Data = "<pre>$Data</pre>"
        Break
    ...
    "/functions/mining/start" { 
        If ($Variables.MiningStatus -ne "Running") { 
            ...
            $Data = "Mining processes starting...`n$(If ($Variables.BalancesTrackerPollInterval -gt 0) { If (-not $BalancesTrackerRunspace) { "Balances Tracker starting..." } Else { "Balances Tracker running." } })"
            $Variables.RestartCycle = $true
        }
        $Data = "<pre>$Data</pre>"
        Break

...
Switch ($Path) { 
        ...
        If ($Pools = @($Variables.Pools.Where({ $_.Name -in $PoolNames -and $_.Algorithm -in $Algorithms }))) { 
            ...
            ForEach ($Pool in $Pools) { 
                If ($PoolsConfig.($Pool.Name).Algorithm -like "-*") { 
                    ...
                    $Pool.Reasons = $Pool.Reasons.Add("Algorithm disabled (`-$($Pool.Algorithm)` in $($Pool.Name) pool config)") | Out-Null | Sort-Object -Unique
                }
                Else { 
                    $PoolsConfig.($Pool.Name).Algorithm = $PoolsConfig.($Pool.Name).Algorithm.Where({ $_ -ne "+$($Pool.Algorithm)" }) | Sort-Object -Unique
                    $Pool.Reasons = $Pool.Reasons.Add("Algorithm not enabled in $($Pool.Name) pool config") | Out-Null | Sort-Object -Unique
                }
                    ...
                    { 
                        ...
                        ForEach ($Algorithm in $_.Algorithms) { 
                            ...
                            If ($Parameters.Value -eq 0) { # Miner failed
                                ...
                                $null = Set-Stat -Name $StatName -Value $Parameters.Value -FaultDetection $false | Out-Null
    ...
    "/miners/best" { 
        $Data = ConvertTo-Json -Depth 4 @($Variables.MinersBest | Sort-Object -Property DeviceNames | Select-Object -ExcludeProperty Arguments, Data, DataReaderJob, DataSampleTimestamp, Devices, EnvVars, PoolNames, Process, ProcessJob, StatEnd, StatStart, SideIndicator, ValidDataSampleTimestamp)
        Break
    }
    "/miners/disabled" { 

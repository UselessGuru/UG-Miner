        ...
        if ($Session.Donation.Running -or $Session.ConfigTimestamp -gt $Session.Timer -or -not $Session.Pools -or $Session.PoolDataCollectedTimeStamp.AddSeconds($Session.Config.Interval / 2) -lt $Session.Timer) { 
            ...
            if ($Session.Config.PoolName) { 
                ...
                if ($Session.Brains.Keys.Where({ $Session.Brains[$_].StartTime -gt $Session.Timer.AddSeconds(- $Session.Config.Interval) }) -or -not $Session.Miners) { 
                    ...
                    $Session.PoolsTimeout = 60
                    $Message = "Loading initial pool data from $((Get-PoolBaseName $Session.Config.PoolName) -join ", " -replace ",([^,]*)$", " &`$1").<br>This may take up to $($Session.PoolsTimeout) seconds..."
            ...
            $MinersNew = ((Get-ChildItem -Path ".\Miners\*.ps1").ForEach(
                    { 
                        ...
                        try { 
                            ...
                            $M = (& $_.ResolvedTarget)
                            $M
                            Write-Message -Level Debug "Miner definition file '$MinerFileName': End building miner objects ($($M.Count) objects)"
                            Remove-Variable M
                        }
                        catch { 

...
$ErrorLogFile = "Logs\$((Get-Item $MyInvocation.MyCommand.Path).BaseName)_Error.txt"
...
Do { 
    ...
    Try {       
        ...
        $Variables.$MyIP = (Get-NetIPAddress -InterfaceIndex (Get-NetConnectionProfile).Where({ $_.IPv4Connectivity -eq "Internet" }).InterfaceIndex -AddressFamily IPV4).IPAddress
        ...
        If ($Variables.EnabledDevices = [Device[]]@($Variables.Devices.Where({ $_.State -ne [DeviceState]::Unsupported -and $_.Name -notin $Config.ExcludeDeviceName }).ForEach({ $_ | Select-Object -Property * }))) { 
            ...
            If ($Variables.CalculatePowerCost = $Config.CalculatePowerCost) { 
                If ($Variables.EnabledDevices.Count -ge 1) { 
                    ...
                    If ($RegValue = Get-ItemProperty -Path $RegistryPath -ErrorAction Ignore) { 
                        ...
                        If ($Variables.HWiNFO64RegTime -eq $HWiNFO64RegTime) { 
                            ...
                            $Variables.Devices.Name.ForEach(
                                { 
                                    ...
                                    If ($ConfiguredPowerConsumption = $Config.PowerConsumption.$_ -as [Double]) { 
                                        If ($Variables.EnabledDevices.Name -contains $_ -and -not $PowerConsumptionData.$_) { Write-Message -Level Warn "HWiNFO64 cannot read power consumption data for device ($_). Using configured value of $ConfiguredPowerConsumption)W." }
            ...
            Else { 
                ...
                If ($Variables.PoolName) { 
                    ...
                    If ($Pools = Compare-Object @($Variables.PoolsNew | Select-Object) @($Variables.Pools.Where({ $Variables.PoolName -contains $_.Variant }) | Select-Object) -Property Algorithm, Currency, Variant -IncludeEqual -PassThru) { 
                        ...
                        If ($Config.Watchdog) { 
                            ...
                            If ($RelevantWatchdogTimers = $Variables.WatchdogTimers.Where({ $_.MinerName -notin $Variables.MinersRunning })) { 
                                ...
                                If ($RelevantPools = $Pools.Where({ $RelevantWatchdogTimers.PoolName -notcontains $_.Name })) { 
        ...
        If ($Variables.DonationRunning -or $Variables.MinersNeedingBenchmark -or $Variables.MinersNeedingPowerConsumptionMeasurement -or (-not $Config.CalculatePowerCost -and $Variables.MiningEarning -ge ($Config.ProfitabilityThreshold / $Variables.Rates.BTC.($Config.FIATcurrency))) -or ($Config.CalculatePowerCost -and $Variables.MiningProfit -ge ($Config.ProfitabilityThreshold / $Variables.Rates.BTC.($Config.FIATcurrency)))) { 
    ...
    Write-Message -Level MemDebug "$((Get-Item $MyInvocation.MyCommand.Path).BaseName) runspace: handles: $($Proc.HandleCount) / memory: $($Proc.PrivateMemorySize64 / 1mb) mb"

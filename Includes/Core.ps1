...
Try { 
    ...
    Do { 
        ...
        $Session.EndCycleTime = If ($Session.EndCycleTime) { $Session.EndCycleTime.AddSeconds($Session.ConfigRunning.Interval) } Else { $Session.BeginCycleTime.AddSeconds($Session.ConfigRunning.Interval) }

        $Session.CycleStarts += $Session.Timer
        $Session.CycleStarts = @($Session.CycleStarts | Sort-Object -Bottom (3, ($Session.ConfigRunning.SyncWindow + 1) | Measure-Object -Maximum).Maximum)

        # Internet connection must be available
        If (-not $Session.MyIPaddress) { 
            $Message = "No internet connection - will retry in $($Session.ConfigRunning.Interval) seconds..."
        ...
        If (-not $Session.ConfigRunning.PoolName) { 
            $Message = "No configured pools - will retry in $($Session.ConfigRunning.Interval) seconds..."
        ...
        $Session.ApplyMinerTweaks = $Session.ConfigRunning.UseMinerTweaks -and $Session.IsLocalAdmin

        # Miner naming scheme has changed. Must clear all existing miners & watchdog timers due to different miner names
        If ($Session.Miners -and $Session.ConfigRunning.BenchmarkAllPoolAlgorithmCombinations -ne $Session.BenchmarkAllPoolAlgorithmCombinations) { 
        ...
        $Session.BenchmarkAllPoolAlgorithmCombinations = $Session.ConfigRunning.BenchmarkAllPoolAlgorithmCombinations
        $Session.PoolTimeout = [Math]::Floor($Session.ConfigRunning.PoolTimeout)

        If ($Session.EnabledDevices = [Device[]]@($Session.Devices.Where({ $_.State -ne [DeviceState]::Unsupported -and $_.Name -notin $Session.ConfigRunning.ExcludeDeviceName }).ForEach({ $_ | Select-Object -Property * }))) { 
            ...
            If ($Session.CalculatePowerCost = $Session.ConfigRunning.CalculatePowerCost) { 
                    ...
                    If ($RegValue = Get-ItemProperty -Path $RegistryPath -ErrorAction Ignore) { 
                        ...
                        Else { 
                            ...
                            $Session.Devices.Name.ForEach(
                                { 
                                    $DeviceName = $_
                                    If ($ConfiguredPowerConsumption = $Session.ConfigRunning.PowerConsumption.$_ -as [Double]) { 
            ...
            # Power price
            If (-not $Session.ConfigRunning.PowerPricekWh.psBase.Keys) { 
                $Session.ConfigRunning.PowerPricekWh."00:00" = 0
            }
            ElseIf ($null -eq $Session.ConfigRunning.PowerPricekWh."00:00") { 
                # 00:00h power price is the same as the latest price of the previous day
                $Session.ConfigRunning.PowerPricekWh."00:00" = $Session.ConfigRunning.PowerPricekWh.($Session.ConfigRunning.PowerPricekWh.psBase.Keys | Sort-Object -Bottom 1)
            }
            $Session.PowerPricekWh = [Double]($Session.ConfigRunning.PowerPricekWh.($Session.ConfigRunning.PowerPricekWh.psBase.Keys.Where({ $_ -le (Get-Date -Format HH:mm).ToString() }) | Sort-Object -Bottom 1))
            $Session.PowerCostBTCperW = [Double](1 / 1000 * 24 * $Session.PowerPricekWh / $Session.Rates.BTC.($Session.ConfigRunning.FIATcurrency))

            # Core suspended with <Ctrl><Alt>P in MainLoop
            While ($Session.SuspendCycle) { Start-Sleep -Seconds 1 }

            # Set minimum Watchdog count 3
            $Session.WatchdogCount = (3, $Session.ConfigRunning.WatchdogCount | Measure-Object -Maximum).Maximum

            # Expire watchdog timers
            $Session.WatchdogReset = $Session.WatchdogCount * $Session.WatchdogCount * $Session.WatchdogCount * $Session.WatchdogCount * $Session.ConfigRunning.Interval
            If ($Session.ConfigRunning.Watchdog) { $Session.WatchdogTimers = $Session.WatchdogTimers.Where({ $_.Kicked -ge $Session.Timer.AddSeconds(- $Session.WatchdogReset) }) }
            ...
            If ($Session.ConfigRunning.Donation -gt 0) { 
                If (-not $Session.DonationStart) { 
                    ...
                    If ((Get-Item -Path "$PWD\Logs\DonationLog.csv" -ErrorAction Ignore).LastWriteTime -lt [DateTime]::Today) { 
                        ...
                        If ($Session.ConfigRunning.Donation -lt (1440 - [Math]::Floor([DateTime]::Now.TimeOfDay.TotalMinutes))) { $Session.DonationStart = [DateTime]::Now.AddMinutes((Get-Random -Minimum 0 -Maximum (1440 - [Math]::Floor([DateTime]::Now.TimeOfDay.TotalMinutes) - $Session.ConfigRunning.Donation))) }
                    }
                }
                ...
                If ($Session.DonationStart -and [DateTime]::Now -ge $Session.DonationStart) { 
                    If (-not $Session.DonationEnd) { 
                        ...
                        $Session.DonationEnd = $Session.DonationStart.AddMinutes($Session.ConfigRunning.Donation)
                        $Session.EndCycleTime = ($Session.DonationEnd).ToUniversalTime()
                        Write-Message -Level Info "Donation run: Mining for '$($Session.DonateUsername)' for the next $(If (($Session.ConfigRunning.Donation - ([DateTime]::Now - $Session.DonationStart).Minutes) -gt 1) { "$($Session.ConfigRunning.Donation - ([DateTime]::Now - $Session.DonationStart).Minutes) minutes" } Else { "minute" })."
            ...
            If ($Session.DonationRunning) { 
                If ($Session.ConfigRunning.Donation -gt 0 -and [DateTime]::Now -lt $Session.DonationEnd) { 
                    # Use donation pool config, use same pool variant to avoid extra benchmarking
                    $Session.ConfigRunning.PoolName = $Session.ConfigRunning.PoolName.Where({ (Get-PoolBaseName $_) -in $Session.DonationPoolsConfig.Keys })
                    $Session.ConfigRunning.PoolsConfig = $Session.DonationPoolsConfig
                }
                Else { 
                    # Donation end
            ...
            If ($Session.DonationRunning -or $Session.ConfigReadTimestamp -gt $Session.Timer -or -not $Session.Pools -or $Session.PoolDataCollectedTimeStamp.AddSeconds($Session.ConfigRunning.Interval / 2) -lt $Session.Timer) { 
                ...
                If (-not $Session.DonationRunning -and $Session.ConfigRunning.AutoUpdateCheckInterval -and $Session.CheckedForUpdate -lt [DateTime]::Now.AddDays(-$Session.ConfigRunning.AutoUpdateCheckInterval)) { Get-Version }

                # Stop / start brain background jobs
                $PoolBaseNames = Get-PoolBaseName $Session.ConfigRunning.PoolName
                ...
                # Collect pool data
                If ($Session.ConfigRunning.PoolName) { 
                    $Session.PoolsCount = $Session.Pools.Count

                    # Wait for pool data message
                    If ($Session.Brains.Keys.Where({ $Session.Brains[$_].StartTime -gt $Session.Timer.AddSeconds(- $Session.ConfigRunning.Interval) })) { 
                        # Newly started brains, allow extra time for brains to get ready
                        $Session.PoolTimeout = 60
                        $Message = "Loading initial pool data from $((Get-PoolBaseName $Session.ConfigRunning.PoolName) -join ", " -replace ",([^,]*)$", " &`$1").<br>This may take up to $($Session.PoolTimeout) seconds..."
                    ...
                    Else { 
                        Write-Message -Level Info "Loading pool data from $((Get-PoolBaseName $Session.ConfigRunning.PoolName) -join ", " -replace ",([^,]*)$", " &`$1")..."
                    }
                    ...
                    $Session.PoolsNew = $Session.ConfigRunning.PoolName.ForEach(
                        { 
                        ...
                        { 
                            ...
                            $Pool.Fee = If ($Session.ConfigRunning.IgnorePoolFee -or $Pool.Fee -lt 0 -or $Pool.Fee -gt 1) { 0 } Else { $Pool.Fee }
                    ...
                    If ($PoolsWithoutData = Compare-Object -PassThru @($Session.ConfigRunning.PoolName) @($Session.PoolsNew.Variant | Sort-Object -Unique)) { Write-Message -Level Warn "No data received from pool$(If ($PoolsWithoutData.Count -gt 1) { "s" }) $($PoolsWithoutData -join ", " -replace ",([^,]*)$", " &`$1")." }
                    ...
                    $Session.Pools = $Session.Pools.Where({ $_.Variant -in $Session.ConfigRunning.PoolName })
                    ...
                    If ($Pools = Compare-Object -PassThru @($Session.PoolsNew | Select-Object) @($Session.Pools.Where({ $Session.ConfigRunning.PoolName -contains $_.Variant }) | Select-Object) -Property Algorithm, Variant -IncludeEqual) { 
                        ...
                        $Pools.ForEach(
                            { 
                                ...
                                $_.PoolPorts = @($(If ($Session.ConfigRunning.SSL -ne "Always" -and $_.Port) { [UInt16]$_.Port } Else { $null }), $(If ($Session.ConfigRunning.SSL -ne "Never" -and $_.PortSSL) { [UInt16]$_.PortSSL } Else { $null }))
                        ...
                        $Pools.Where({ $_.Accuracy -lt $Session.ConfigRunning.MinAccuracy }).ForEach({ $_.Reasons.Add("MinAccuracy ($($Session.ConfigRunning.MinAccuracy * 100)%) not reached") | Out-Null })
                        # Filter unavailable algorithms
                        If ($Session.ConfigRunning.MinerSet -lt 3) { $Pools.Where({ $Session.UnprofitableAlgorithms[$_.Algorithm] -eq "*" }).ForEach({ $_.Reasons.Add("Unprofitable algorithm") | Out-Null }) }
                        # Pool price 0
                        $Pools.Where({ $_.Price -eq 0 -and -not ($Session.ConfigRunning.PoolsConfig[$_.Name].PoolAllow0Price -or $Session.ConfigRunning.PoolAllow0Price) }).ForEach({ $_.Reasons.Add("Price -eq 0") | Out-Null })
                        # No price data
                        $Pools.Where({ [Double]::IsNaN($_.Price) }).ForEach({ $_.Reasons.Add("Price information not available") | Out-Null })
                        # Ignore pool if price is more than $Session.ConfigRunning.UnrealisticPoolPriceFactor higher than the medium price of all pools with same algorithm; NiceHash & MiningPoolHub are always right
                        If ($Session.ConfigRunning.UnrealisticPoolPriceFactor -gt 1) { 
                            ($Pools.Where({ $_.Price_Bias -gt 0 }) | Group-Object -Property Algorithm).Where({ $_.Count -gt 3 }).ForEach(
                                { 
                                    If ($PriceThreshold = (Get-Median $_.Group.Price_Bias) * $Session.ConfigRunning.UnrealisticPoolPriceFactor) { 
                                        $_.Group.Where({ $_.Name -notin @("NiceHash", "MiningPoolHub") -and $_.Price_Bias -gt $PriceThreshold }).ForEach({ $_.Reasons.Add("Unrealistic price (more than $($Session.ConfigRunning.UnrealisticPoolPriceFactor)x higher than median price)") | Out-Null })
                                    }
                                }
                            )
                        ...
                        $Pools.Where({ $Session.ConfigRunning.PoolsConfig[$_.Name].Algorithm -like "+*" -and $Session.ConfigRunning.PoolsConfig[$_.Name].Algorithm -split "," -notcontains "+$($_.AlgorithmVariant)" -and $Session.ConfigRunning.PoolsConfig[$_.Name].Algorithm -split "," -notcontains "+$($_.Algorithm)" }).ForEach({ $_.Reasons.Add("Algorithm not enabled in $($_.Name) pool config") | Out-Null })
                        ...
                        ElseIf ($Session.ConfigRunning.Algorithm -like "-*") { 
                            $ExcludeAlgorithmNames = @($Session.ConfigRunning.Algorithm -replace "^-" | Select-Object)
                        ...
                        $Pools.Where({ $Session.ConfigRunning.PoolsConfig[$_.Name].Currency -like "+*" -and $Session.ConfigRunning.PoolsConfig[$_.Name].Currency -split "," -notcontains "+$($_.Currency)" }).ForEach({ $_.Reasons.Add("Currency not enabled in $($_.Name) pool config") | Out-Null })
                        $Pools.Where({ $Session.ConfigRunning.PoolsConfig[$_.Name].Currency -split "," -contains "-$($_.Currency)" }).ForEach({ $_.Reasons.Add("Currency disabled (``-$($_.Currency)`` in $($_.Name) pool config)") | Out-Null })
                        # Filter non-enabled currencies
                        If ($Session.ConfigRunning.Currency -like "+*") { $Pools.Where({ $Session.ConfigRunning.Currency -split "," -notcontains "+$($_.Currency)" }).ForEach({ $_.Reasons.Add("Currency not enabled in generic config") | Out-Null }) }
                        # Filter disabled currencies
                        ElseIf ($Session.ConfigRunning.Currency -like "-*") { $Pools.Where({ $Session.ConfigRunning.Currency -split "," -contains "-$($_.Currency)" }).ForEach({ $_.Reasons.Add("Currency disabled (``-$($_.Currency)`` in generic config)") | Out-Null }) }
                        # MinWorkers
                        $Pools.Where({ $null -ne $_.Workers -and $_.Workers -lt $Session.ConfigRunning.PoolsConfig[$_.Name].MinWorkers }).ForEach({ $_.Reasons.Add("Not enough workers at pool (MinWorker ``$($Session.ConfigRunning.PoolsConfig[$_.Name].MinWorker)`` in $($_.Name) pool config)") | Out-Null })
                        $Pools.Where({ $null -ne $_.Workers -and $_.Workers -lt $Session.ConfigRunning.MinWorker -and $Session.ConfigRunning.PoolsConfig[$_.Name].MinWorkers -ne 0 -and $Session.ConfigRunning.PoolsConfig[$_.Name].MinWorkers -lt $Session.ConfigRunning.MinWorker }).ForEach({ $_.Reasons.Add("Not enough workers at pool (MinWorker ``$($Session.ConfigRunning.MinWorker)`` in generic config)") | Out-Null })
                        # SSL
                        $Pools.Where({ $Session.ConfigRunning.PoolsConfig[$_.Name].SSL -eq "Never" }).ForEach({ $_.PoolPorts[1] = $null })
                        $Pools.Where({ $Session.ConfigRunning.PoolsConfig[$_.Name].SSL -eq "Always" }).ForEach({ $_.PoolPorts[0] = $null })
                        $Pools.Where({ $Session.ConfigRunning.PoolsConfig[$_.Name].SSL -eq "Never" -and -not $_.PoolPorts[0] }).ForEach({ $_.Reasons.Add("Non-SSL port not available (SSL -eq 'Never' in $($_.Name) pool config)") | Out-Null })
                        $Pools.Where({ $Session.ConfigRunning.PoolsConfig[$_.Name].SSL -eq "Always" -and -not $_.PoolPorts[1] }).ForEach({ $_.Reasons.Add("SSL port not available (SSL -eq 'Always' in $($_.Name) pool config)") | Out-Null })
                        If ($Session.ConfigRunning.SSL -eq "Never") { $Pools.Where({ -not $_.PoolPorts[0] -and $_.Reasons -notmatch "Non-SSL port not available .+" }).ForEach({ $_.Reasons.Add("Non-SSL port not available (SSL -eq 'Never' in generic config)") | Out-Null }) }
                        ElseIf ($Session.ConfigRunning.SSL -eq "Always") { $Pools.Where({ -not $_.PoolPorts[1] -and $_.Reasons -notmatch "SSL port not available .+" }).ForEach({ $_.Reasons.Add("SSL port not available (SSL -eq 'Always' in generic config)") | Out-Null }) }
                        # SSL Allow selfsigned certificate
                        $Pools.Where({ $_.SSLselfSignedCertificate -and $null -ne $Session.ConfigRunning.PoolsConfig[$_.Name].SSLallowSelfSignedCertificate -and $Session.ConfigRunning.PoolsConfig[$_.Name].SSLallowSelfSignedCertificate -eq $false }).ForEach({ $_.Reasons.Add("Pool uses self signed certificate (SSLallowSelfSignedCertificate -eq '`$false' in $($_.Name) pool config)") | Out-Null })
                        If (-not $Session.ConfigRunning.SSLallowSelfSignedCertificate) { $Pools.Where({ $_.SSLselfSignedCertificate -and $null -eq $Session.ConfigRunning.PoolsConfig[$_.Name].SSLallowSelfSignedCertificate }).ForEach({ $_.Reasons.Add("Pool uses self signed certificate (SSLallowSelfSignedCertificate -eq '`$false' in generic config)") | Out-Null }) }
                        ...
                        If ($Session.ConfigRunning.MinerSet -le 2) { 
                        ...
                        If ($Session.ConfigRunning.BalancesKeepAlive -and $Global:BalancesTrackerRunspace -and $Session.PoolsLastEarnings.Count -gt 0 -and $Session.PoolsLastUsed) { 
                            $Session.ConfigRunning.PoolNamesToKeepBalancesAlive = @()
                            ForEach ($Pool in @($Pools.Where({ $_.Name -notin $Session.ConfigRunning.BalancesTrackerExcludePool }) | Sort-Object -Property Name -Unique)) { 
                                If ($Session.PoolsLastEarnings[$Pool.Name] -and $Session.ConfigRunning.PoolsConfig[$Pool.Name].BalancesKeepAlive -gt 0 -and ([DateTime]::Now.ToUniversalTime() - $Session.PoolsLastEarnings[$Pool.Name]).Days -ge ($Session.ConfigRunning.PoolsConfig[$Pool.Name].BalancesKeepAlive - 10)) { 
                                    $Session.ConfigRunning.PoolNamesToKeepBalancesAlive += $Pool.Name
                                    Write-Message -Level Warn "Pool '$($Pool.Name)' prioritized to avoid forfeiting balance (pool would clear balance in 10 days)."
                                }
                            }
                            Remove-Variable Pool

                            If ($Session.ConfigRunning.PoolNamesToKeepBalancesAlive) { 
                                $Pools.ForEach(
                                    { 
                                        If ($Session.ConfigRunning.PoolNamesToKeepBalancesAlive -contains $_.Name) { $_.Available = $true; $_.Prioritize = $true }
                        ...
                        ($Pools.Where({ $_.Available }) | Group-Object -Property Algorithm).ForEach({ ($_.Group | Sort-Object -Property Prioritize, Price_Bias -Bottom $(If ($Session.ConfigRunning.MinerUseBestPoolsOnly -or $_.Group.Algorithm -notmatch $Session.RegexAlgoHasDAG) { 1 } Else { $_.Group.Count })).ForEach({ $_.Best = $true }) })
                    }
            ...
            If (-not $Session.PoolsBest) { 
                $Message = "No minable pools - will retry in $($Session.ConfigRunning.Interval) seconds..."
                Write-Message -Level Warn $Message
                $Session.Summary = $Message
                Remove-Variable Message
            ...
            ForEach ($Miner in $Session.MinersBest) { 
                ...
                If ([MinerStatus]::DryRun, [MinerStatus]::Running -contains $Miner.Status) { 
                    If ($Miner.Status -eq [MinerStatus]::DryRun -or $Miner.GetStatus() -eq [MinerStatus]::Running) { 
                        ...
                        If ($Miner.Status -eq [MinerStatus]::Running -and $Session.ConfigRunning.BadShareRatioThreshold -gt 0) { 
                            If ($Shares = ($Miner.Data | Select-Object -Last 1).Shares) { 
                                ForEach ($Algorithm in $Miner.Algorithms) { 
                                    If ($Shares.$Algorithm -and $Shares.$Algorithm[1] -gt 0 -and $Shares.$Algorithm[3] -gt [Math]::Floor(1 / $Session.ConfigRunning.BadShareRatioThreshold) -and $Shares.$Algorithm[1] / $Shares.$Algorithm[3] -gt $Session.ConfigRunning.BadShareRatioThreshold) { 
                ...
                If ($Miner.Activated -gt 0 -and $Miner.Status -ne [MinerStatus]::DryRun) { 
                    ...
                    If ($Miner.Data.Count -ge $Miner.MinDataSample -or $Miner.Activated -gt $Session.WatchdogCount) { 
                        ...
                        ForEach ($Worker in $Miner.Workers) { 
                            ...
                            If ($Miner.Data.Count -gt $Miner.MinDataSample -and -not $Miner.Benchmark -and $Session.ConfigRunning.SubtractBadShares -and $MinerData.$Algorithm -gt 0) { 
            ...
            If ($AvailableMinerPools = If ($Session.ConfigRunning.MinerUseBestPoolsOnly) { $Session.Pools.Where({ $_.Available -and ($_.Best -or $_.Prioritize) }) } Else { $Session.Pools.Where({ $_.Available }) }) { 
                ...
                $MinersNew = ((Get-ChildItem -Path ".\Miners\*.ps1").ForEach(
                    ...
                    { 
                        ...
                        Try { 
                            ForEach ($Worker in $Miner.Workers) { 
                                $Miner.Workers[$Miner.Workers.IndexOf($Worker)].Fee = If ($Session.ConfigRunning.IgnoreMinerFee) { 0 } Else { $Miner.Fee[$Miner.Workers.IndexOf($Worker)] }
                            }
            ...
            If (-not $Session.ConfigRunning.PoolAllow0Price) { $Miners.Where({ $_.Earnings -eq 0 }).ForEach({ $_.Reasons.Add("Earnings -eq 0") | Out-Null }) }
            $Miners.Where({ -not $_.Benchmark -and $_.Workers.Hashrate -contains 0 }).ForEach({ $_.Reasons.Add("0 H/s stat file") | Out-Null })
            If ($Session.ConfigRunning.DisableMinersWithFee) { $Miners.Where({ $_.Workers.Fee }).ForEach({ $_.Reasons.Add("Config.DisableMinersWithFee") | Out-Null }) }
            If ($Session.ConfigRunning.DisableDualAlgoMining) { $Miners.Where({ $_.Workers.Count -eq 2 }).ForEach({ $_.Reasons.Add("Config.DisableDualAlgoMining") | Out-Null }) }
            If ($Session.ConfigRunning.DisableSingleAlgoMining) { $Miners.Where({ $_.Workers.Count -eq 1 }).ForEach({ $_.Reasons.Add("Config.DisableSingleAlgoMining") | Out-Null }) }
            ...
            If ($Miners.Where({ $_.Available })) { 
                Write-Message -Level Info "Selecting best miner$(If (($Session.EnabledDevices.Model | Select-Object -Unique).Count -gt 1) { " combinations" }) based on$(If ($Session.CalculatePowerCost -and -not $Session.ConfigRunning.IgnorePowerCost) { " profit (power cost $($Session.ConfigRunning.FIATcurrency) $($Session.PowerPricekWh)/kW⋅h)" } Else { " earnings" })..."
                ...
                Else { 
                    ...
                    $RunningMinerBonusFactor = 1 + $Session.ConfigRunning.MinerSwitchingThreshold / 100
                ...
                $Session.PowerConsumptionIdleSystemW = (($Session.ConfigRunning.PowerConsumptionIdleSystemW - ($MinersBest.Where({ $_.Type -eq "CPU" }) | Measure-Object PowerConsumption -Sum).Sum), 0 | Measure-Object -Maximum).Maximum
                $Session.BasePowerCost = [Double]($Session.PowerConsumptionIdleSystemW / 1000 * 24 * $Session.PowerPricekWh / $Session.Rates.BTC.($Session.ConfigRunning.FIATcurrency))
            ...
            Else { 
                $Session.PowerConsumptionIdleSystemW = (($Session.ConfigRunning.PowerConsumptionIdleSystemW), 0 | Measure-Object -Maximum).Maximum
                $Session.BasePowerCost = [Double]($Session.PowerConsumptionIdleSystemW / 1000 * 24 * $Session.PowerPricekWh / $Session.Rates.BTC.($Session.ConfigRunning.FIATcurrency))
        ...
        Else { 
            $Message = "No enabled devices - will retry in $($Session.ConfigRunning.Interval) seconds..."
            Write-Message -Level Warn $Message
            $Session.Summary = $Message
            Remove-Variable Message
        ...
        $Summary = ""
        If ($Session.Rates.($Session.ConfigRunning.PayoutCurrency)) { 
            $PayoutCurrency = If ($Session.ConfigRunning.PayoutCurrency -eq "BTC" -and $Session.ConfigRunning.UsemBTC) { "mBTC" } Else { $Session.ConfigRunning.PayoutCurrency }
            # ProfitabilityThreshold check - OK to run miners?
            If ($Session.CalculatePowerCost -and ($Session.MiningProfit * $Session.Rates.BTC.($Session.ConfigRunning.FIATcurrency)) -lt $Session.ConfigRunning.ProfitabilityThreshold) { 
                # Mining earnings/profit is below threshold
                $MinersBest = [Miner[]]@()
                $Text = "Mining profit of {0} {1:n} / day is below the configured threshold of {0} {2:n} / day. Mining is suspended until the threshold is reached." -f $Session.ConfigRunning.FIATcurrency, ($Session.MiningProfit * $Session.Rates.BTC.($Session.ConfigRunning.FIATcurrency)), $Session.ConfigRunning.ProfitabilityThreshold
            ...
            Else { 
                ...
                ElseIf ($Session.MiningEarnings -gt 0) { 
                    $Summary += "Earnings / day: {0:n} {1} ({2:N$(Get-DecimalsFromValue -Value ($Session.MiningProfit * ($Session.MiningProfit * $Session.Rates.BTC.$PayoutCurrency)) -DecimalsMax $Session.ConfigRunning.DecimalsMax)} {3})" -f ($Session.MiningEarnings * $Session.Rates.BTC.($Session.ConfigRunning.FIATcurrency)), $Session.ConfigRunning.FIATcurrency, ($Session.MiningEarnings * $Session.Rates.BTC.$PayoutCurrency), $PayoutCurrency
                }
                ...
                If ($Session.CalculatePowerCost) { 
                    ...
                    ElseIf ($Session.MiningPowerConsumption -gt 0) { 
                        $Summary += "    Profit / day: {0:n} {1} ({2:N$(Get-DecimalsFromValue -Value ($Session.MiningProfit * $Session.Rates.BTC.$PayoutCurrency) -DecimalsMax $Session.ConfigRunning.DecimalsMax)} {3})" -f ($Session.MiningProfit * $Session.Rates.BTC.($Session.ConfigRunning.FIATcurrency)), $Session.ConfigRunning.FIATcurrency, ($Session.MiningProfit * $Session.Rates.BTC.$PayoutCurrency), $PayoutCurrency
                    }
                    Else { 
                    ...
                    If ([Double]::IsNaN($Session.MiningEarnings) -or [Double]::IsNaN($Session.MiningPowerCost)) { 
                        $Summary += "`nPower cost / day: n/a [Miner$(If ($MinersBest.Count -ne 1) { "s" }): n/a; Base: {1:n} {0} ({2:n2} W)]" -f $Session.ConfigRunning.FIATcurrency, ($Session.BasePowerCost * $Session.Rates.BTC.($Session.ConfigRunning.FIATcurrency)), $Session.PowerConsumptionIdleSystemW
                    }
                    ElseIf ($Session.MiningPowerConsumption -gt 0) { 
                        $Summary += "`nPower cost / day: {1:n} {0} [Miner$(If ($MinersBest.Count -ne 1) { "s" }): {2:n} {0} ({3:n2} W)$(If ($Session.PowerConsumptionIdleSystemW) { "; Base: {4:n} {0} ({5:n2} W)]" })" -f $Session.ConfigRunning.FIATcurrency, (($Session.MiningPowerCost + $Session.BasePowerCost) * $Session.Rates.BTC.($Session.ConfigRunning.FIATcurrency)), ($Session.MiningPowerCost * $Session.Rates.BTC.($Session.ConfigRunning.FIATcurrency)), $Session.MiningPowerConsumption, ($Session.BasePowerCost * $Session.Rates.BTC.($Session.ConfigRunning.FIATcurrency)), $Session.PowerConsumptionIdleSystemW
                    }
                    Else { 
                        $Summary += "`nPower cost / day: n/a [Miner: n/a$(If ($Session.PowerConsumptionIdleSystemW) { "; Base: {1:n} {0} ({2:n2} W)]" })" -f $Session.ConfigRunning.FIATcurrency, ($Session.BasePowerCost * $Session.Rates.BTC.($Session.ConfigRunning.FIATcurrency)), $Session.PowerConsumptionIdleSystemW
                    }
                }
            }
            ...
            ((@(If ($Session.ConfigRunning.UsemBTC) { "mBTC" } Else { ($Session.ConfigRunning.PayoutCurrency) }) + @($Session.ConfigRunning.ExtraCurrencies)) | Select-Object -Unique).Where({ $Session.Rates.$_.($Session.ConfigRunning.FIATcurrency) }).ForEach(
                { 
                    $Summary += "1 $_ = {0:N$(Get-DecimalsFromValue -Value $Session.Rates.$_.($Session.ConfigRunning.FIATcurrency) -DecimalsMax $Session.ConfigRunning.DecimalsMax)} $($Session.ConfigRunning.FIATcurrency)   " -f $Session.Rates.$_.($Session.ConfigRunning.FIATcurrency)
                }
            )
        ...
        Else { 
            $Message = "Error: Could not get BTC exchange rate from 'min-api.cryptocompare.com' for currency '$($Session.ConfigRunning.PayoutCurrency)'. Cannot determine best miners to run - will retry in $($Session.ConfigRunning.Interval) seconds..."
            Write-Message -Level Warn $Message
            $Session.Summary = $Message
            Remove-Variable Message
        ...
        While ($StuckMinerProcesses = (Get-CimInstance CIM_Process).Where({ $_.ExecutablePath -and $MinerPaths -contains $_.ExecutablePath -and $Miners.ProcessID -notcontains $_.ProcessID -and $Miners.ProcessID -notcontains $_.ParentProcessID}).ProcessId.ForEach({ (Get-Process -Id $_ -ErrorAction Ignore).Where({ $_.MainWindowTitle -match ".+ \{.+@.+\}" })})) { 
            ...
            $Loops ++
            If ($Loops -gt 50) { 
                If ($Session.ConfigRunning.AutoReboot) { 
        ...
        If (-not $Session.MinersBest) { 
            $Message = "No profitable miners - will retry in $($Session.ConfigRunning.Interval) seconds..."
            Write-Message -Level Warn $Message
            $Session.Summary = $Message
            Remove-Variable Message
        ...
        Start-Sleep -Seconds $Session.ConfigRunning.Delay

        ForEach ($Miner in $Session.MinersBest) { 

            $DataCollectInterval = If ($Miner.Benchmark -or $Miner.MeasurePowerConsumption) { If ($Session.ConfigRunning.DryRun -and $Session.ConfigRunning.BenchmarkAllPoolAlgorithmCombinations) { 0.5 } Else { 1 } } Else { 5 }

            If ($Miner.Status -ne [MinerStatus]::DryRun -and $Miner.GetStatus() -ne [MinerStatus]::Running) { 
                If ($Session.ConfigRunning.DryRun -and -not ($Miner.Benchmark -or $Miner.MeasurePowerConsumption)) { 
        ...
        If ($Session.CycleStarts.Count -eq 1) { $Session.EndCycleTime = [DateTime]::Now.ToUniversalTime().AddSeconds($Session.ConfigRunning.Interval) }

        Do { 
            $LoopEnd = If ($Session.ConfigRunning.DryRun -and $Session.ConfigRunning.BenchmarkAllPoolAlgorithmCombinations) { [DateTime]::Now.AddSeconds(0.5) } Else { [DateTime]::Now.AddSeconds(1) }
                    ...
                    Else { 
                        ...
                        If ($Miner.DataReaderJob.HasMoreData) { 
                            If ($Samples = @($Miner.DataReaderJob | Receive-Job).Where({ $_.Hashrate.PSObject.Properties.Name })) { 
                                ...
                                ElseIf (-not $Session.ConfigRunning.Ignore0HashrateSample -or $Miner.ValidDataSampleTimestamp -ne [DateTime]0) { 
                                    Write-Message -Level Verbose "$($Miner.Name) data sample discarded [$(($Sample.Hashrate.PSObject.Properties.Name.ForEach({ "$($_): $(([Double]$Sample.Hashrate.$_ | ConvertTo-Hash) -replace " ")$(If ($Session.ConfigRunning.ShowShares) { " (Shares: A$($Sample.Shares.$_[0])+R$($Sample.Shares.$_[1])+I$($Sample.Shares.$_[2])=T$($Sample.Shares.$_[3]))" })" })) -join " & ")$(If ($Sample.PowerConsumption) { " | Power: $($Sample.PowerConsumption.ToString("N2"))W" })]$(If ($Miner.ValidDataSampleTimestamp -ne [DateTime]0) { " (Miner is warming up [$(([DateTime]::Now.ToUniversalTime() - $Miner.ValidDataSampleTimestamp).TotalSeconds.ToString("0") -replace "-0", "0") sec])" })"
